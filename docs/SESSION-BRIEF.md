...
- **Architecture = Approach B+:** Polygon mainnet (137) = execution; Arc Testnet (5042002) = account/NAV layer only. **Entry spine is now same-chain Polygon (137)** — one LI.FI Composer signature swaps native USDC→USDC.e and fans across the legs (no bridge). Ethereum (1)/Base (8453) cross-chain is a **flagged stretch** (`NEXT_PUBLIC_ENABLE_CROSSCHAIN_ENTRY`), **never Arc** (Arc→Polygon LI.FI route is verified dead, `{connections:[]}`).
- **It is LIVE.** Real on-chain artifacts:
  ...
  - **Dashboard pulls all-live data:** Polymarket Gamma odds + NVDA price + free **Yahoo analyst price-target bands** + Uniswap `/quote` → the AI Sentiment Gap, every value tagged `live` (seeded fallback).
  - **On-chain asset sleeve:** each bucket buys deeply-liquid Polygon tokens (AI = WETH + LINK; others WETH/WBTC/LINK) via `EnterBasket.enterAssetLeg` → Uniswap `SwapRouter02`, slippage-floored by `/quote` (server route `/api/basket-entry` refuses a 0-minOut swap), swept to the user. NVDA stays the analyst-band anchor.
  - **Arc USDC-gas userOp:** the Account panel has a one-click **Send USDC-gas test op** (`paymaster: true`) on Arc Testnet (Paymaster verified live: v0.8 `0x3BA9…8966`) — `sendArcGaslessUserOp` in `lib/arc/wallet.ts`.
- **Tests/build green:** **82 vitest unit tests**; **7/7 Foundry fork tests** (incl. a REAL USDC.e→WETH SwapRouter02 swap inside `enterAssetLeg`) on a Polygon fork; `tsc` clean; `next build` green (incl. `/api/basket-entry`).
...
## STEP 6 — What remains (live ops + polish — see `docs/task-9-live-qualification.md`)
All three tracks are demo-ready from the UI; the remaining items are **live ops** (your wallet/keys): (1) submit the **Uniswap Developer Feedback Form** with the swap hash (required for $7k); (2) execute **one live same-chain (137) LI.FI route** from a Polygon wallet holding native USDC — record the hash (no bridge on the same-chain spine, so the old cross-chain amount/fee-tuning caveat no longer applies; it remains only for the flagged cross-chain stretch); (3) fund the Arc smart account at `faucet.circle.com` and click **Send USDC-gas test op** — record the ArcScan hash; (4) record the 90s demo video (`docs/demo-script.md`).
