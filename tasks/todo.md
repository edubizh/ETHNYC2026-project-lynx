...
- [ ] **Slice A (pure, TDD):** `lib/mindshare.ts` — add `Window`/`WINDOWS`, `composeIdx(bandPct,beliefProb)`, `aggregateMindshare(bucketInputs)` (mindshare% ∝ activity, +Others reserve = 100, ranked desc), `seedMindshareView()`. Keep SEED/getMindshare/tileCode/getBucketMeta client-safe (no adapter imports). New `test/mindshare.test.ts`.
- [ ] **Slice B (adapters, TDD):** `lib/adapters/polymarket.ts` `fetchMarketVolumes(gammaId)` → all-window volumes (one Gamma fetch); `lib/adapters/equities.ts` `fetchEquityQuote(symbol)` → `{price,changePct}` (Finnhub `c`+`dp`), `fetchEquityPrice` delegates. Extend `test/adapters.test.ts` + `test/equities.test.ts`.
- [ ] **Slice C (orchestration, TDD):** `lib/mindshare-live.ts` (server-only — imports adapters/config) `buildMindshareWindows(): Promise<WindowsView>`; per-bucket parallel fetch (leg volumes + primary belief + headline quote), each `withFallback` to seed; whole-failure → `seedMindshareView()`. New `test/mindshare-live.test.ts` (mocked adapter spies).
- [ ] **Slice D (UI):** `app/page.tsx` → async server component fetching `buildMindshareWindows()`, renders client `components/Browse.tsx` (moves chrome+treemap; `useState<Window>` toggle pills; `SearchProvider` shares query with `TopBar`). `components/TopBar.tsx` search input → `useSearch()`; filters by theme title / security ticker; empty state; search-active shows filtered grid, empty search keeps locked treemap. Monochrome preserved.
- [ ] **Verify:** `npx tsc --noEmit`, `npx vitest run` (≥35+ green), curl `/` + `/theme/ai` = 200, push to main.
