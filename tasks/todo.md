...
- [x] **Arc qualification artifact (Circle MCP-confirmed + wired):** Arc Testnet Circle **Paymaster is LIVE** (v0.8 `0x3BA9…8966`, v0.7 `0x31BE…0b58`); **CCTP Arc→Polygon-mainnet is impossible** (testnet/mainnet) → use the Paymaster userOp. Wired `sendArcGaslessUserOp` (`lib/arc/wallet.ts`, TDD) + `ArcProvider.sendGaslessUserOp` + **Send USDC-gas test op** button (AccountPanel) → ArcScan link. Memory `arc-qualification-artifact`.
- [x] **Yahoo analyst bands (merged from feat/yahoo-analyst-bands):** free live analyst price-target bands w/ hardcoded fallback (`lib/adapters/yahoo.ts`, `yahoo-finance2`); integrated into `buildDashboard`.
- **Green-bar (current):** **82/82 vitest**, tsc clean, `next build` green (incl. `/api/basket-entry`), **7/7 forge fork tests** (incl. REAL USDC.e→WETH swap inside `enterAssetLeg`). origin/main up to date.
- [ ] **Task 9 (USER / live — runbook `docs/task-9-live-qualification.md`):** execute ONE live LI.FI same-chain (137) route from a Polygon wallet holding native USDC; click **Send USDC-gas test op** on Arc Testnet (fund via faucet.circle.com); submit Uniswap Developer Feedback Form with swap `0x23a0…cbde`; record hashes in README.
